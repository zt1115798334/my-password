...
public class DateUtils {
    ...
    public static String formatTime(LocalTime time) {
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(TIME_FORMAT_3);
     ...
     * @return LocalDateTime
     */
    public static LocalDateTime currentDateTimeAddTime(long num, TimeUnit timeUnit) {
        LocalDateTime currentDateTime = LocalDateTime.now();
        if (Objects.equal(timeUnit, TimeUnit.DAYS)) {
            return currentDateTime.plusDays(num);
        }
        if (Objects.equal(timeUnit, TimeUnit.HOURS)) {
            return currentDateTime.plusHours(num);
        }
        if (Objects.equal(timeUnit, TimeUnit.MINUTES)) {
    ...
    public static LocalDateTime currentDateTimeToFirstTime() {
        return dateTimeToFirstTime(LocalDateTime.now());
    }
    ...
    public static LocalDateTime currentDateTimeToLastTime() {
        return dateTimeToLastTime(LocalDateTime.now());
    }

...
import com.zt.mypassword.utils.XssUtils;
    ...
    private JSONObject jsonObjectCleanXSS(JSONObject jsonObj) {
        ...
        for (Map.Entry<String, Object> entry : jsonObj.entrySet()) {
            ...
            if (value instanceof JSONArray) {
                ...
                List<JSONObject> collect = jsonArray.stream().map(object -> {
                    JSONObject j = TypeUtils.castToJavaBean(object, JSONObject.class);
                    return jsonObjectCleanXSS(j);
                }).collect(Collectors.toList());
                resultObj.put(entry.getKey(), collect);
            } else if (value instanceof JSONObject) {
                jsonObjectCleanXSS(TypeUtils.castToJavaBean(value, JSONObject.class));
            } else {
